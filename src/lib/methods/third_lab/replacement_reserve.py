...
def get_replacement_reserve_probability(lambda_: float, m: int, t: float) -> float:
    """
    lam = заданные лямбды
    m = кратность резервирования[1:] (число резервируемых элементов +1)
    """
    total_sum = 0
    for i in range(m):
        total_sum += pow(lambda_ * t, i) / math.factorial(i)

    return math.exp(-lambda_ * t) * total_sum
...
def calculate(request: methods_third_lab_models.ThirdLabModel):
    ...
    for lambda_, reserve in zip(
        ...
        probabilities.append(get_replacement_reserve_probability(lambda_, reserve, request.t))
